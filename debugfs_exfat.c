...
void add_node(char *dir, int len, unsigned long long de_addr)
{
    ...
    //printf("[DEBUG][%s] current : 0x%p, dir:%s, len:%d, addr:0x%08X\n", __func__, current, dir, len, de_addr);
    if (current == NULL) {
        ...
        new->prev = current;
        current->next = new;
        //printf("[DEBUG] sub node. path:%s, addr:0x%08X\n", new->pwd, new->de_addr);
    }

    if (current->next != NULL) {
        current = current->next; // current will indicate last one always.
    }
    //printf("[DEBUG][%s] current : 0x%p\n", __func__, current);
}
